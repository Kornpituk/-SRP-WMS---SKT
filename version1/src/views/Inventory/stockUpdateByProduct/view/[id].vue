...
const GetProduct = () => {
  ...
  axiosIns.get(`${urlApi.value}/api/v1/StockUpdate/ByProduct?page=`+currentPage.value+`&perPage=`+rowPerPage.value+`&productId=`+productId.value+`&unitId=`+UnitId.value, {
    headers: {
      'accept': '*/*',
    ...
    .then(response => {
      ...
      console.log('product response.data view: ', productsLocation)
      ...
      // console.log('productsLocation.value: ',productsLocation)
...
const numberWithCommas = number => {
  ...
  if (number !== undefined && number !== null && configsShowDigit == 'false') {
    const numberString = number.toString()
    
    // กำหนดขนาดตัวหนังสือตามเงื่อนไข
    const fontSize = numberString.length  >=7 ? 'smaller-font-size' : 'normal-font-size'
    
    return `${numberString.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`
  } else if (number !== undefined && number !== null && configsShowDigit == 'true') {
    const numberString = number.toString()
    
    // กำหนดขนาดตัวหนังสือตามเงื่อนไข
    const fontSize = numberString.length  >=7 ? 'smaller-font-size' : 'normal-font-size'
    const resualt = Math.ceil(numberString.replace(/\B(?=(\d{3})+(?!\d))/g, ','))
...
const stockUpdateExcel = () => {
  ...
  axiosIns.post(`${urlApi.value}/api/v1/StockUpdate/ByProduct/Excel?productId=`+productId.value+`&unitId=`+UnitId.value, {}, {
    headers: {
      'accept': '*/*',
      'x-location': `${whereHouse.value}`,
      Authorization: `Bearer ${accessTokenAtStore}`,
    ...
    .then(response => {
      ...
      const fileName = `stock_update_By_Product_${productsLocation.value.productName}_Tag_${dateString}.xlsx` // ตั้งชื่อไฟล์โดยรวมกับวันที่
...
const configExpiryDate = ref(localStorage.getItem('configsShowExpiryDate'))

console.log('configsShowExpiryDate in view', configExpiryDate.value)

// console.log('Company Name:', nameCompany)
...
const checkSeritalNo = () => {
  if (configExpiryDate.value === 'true') {
  ...
  <section v-if="false">
    <VRow>
      <VCol
        ...
        lg="4"
      >
        <RouterLink :to="{ name: 'inventory-stockUpdate', query: $route.query }">
          <IconBtn>
            <VIcon
              icon="
                mdi-chevron-left"
                ...
                >
                  ...
                  >
                    <span
                      ...
                      :class="{ 'smaller-font-size': numberWithCommas(productsLocation.qty).length >= 7 }"
                    ><span v-if="productsLocation.qty">{{ (formatDecimal(productsLocation.qty)).toLocaleString('en-US') }}</span> {{ productsLocation.unitName }}</span>
                  </VCard>
                </VCol>
        ...
        <tbody style="text-align: center;">
          ...
          >
            ...
            <td
              ...
              class="text-start px-1"
            >
              {{ formatDateTime(product.expireDate) }}
            </td>
            ...
            >
              <RouterLink
                :to="{ 
                  name: 'inventory-stockUpdateByProduct-lot-id', 
                  params: { id: index }, 
                  query: { productId: productIdRoute ,
                           UnitId: product.unitId,
                           warehouseStock:whereHouse,
                           LotMas:product.lotMaster,
                           ZoneId:product.zoneId,
                           AreaId:product.areaId,
                           subAreaId:product.subAreaId,
                           serialCode: serialProduct,
                  }, 
                }"
              >
...
.normal-font-size {
  font-size: inherit; /* หรืออื่น ๆ ตามที่คุณต้องการ */
}
</style>
