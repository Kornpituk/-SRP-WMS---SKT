...
const exportButtonFunction = () => {
  ...
  // console.log('exportButtonFunction in exportButtonFunction')
}
...
const responseDataForRouteParams = ref([])
...
const getCountingStock = () => {
  ...
  axiosIns.get(`${urlApi.value}/api/v1/CountingStock/SearchCounting?`, {
    ...
    .then(response => {
      ...
      responseDataForRouteParams.value = response.data

      // console.log('responseDataForRouteParams:', responseDataForRouteParams)
...
const countingStockStart = async () => {
  try {
    ...
    StockAdjustIdId.value = response.data.stockAdjustId
    statusAdjustStock.value = response.data.statusName
    console.log('Response:', response)
    console.log('response.data.statusName:', statusAdjustStock.value)
    console.log('response.data.stockAdjustId:', StockAdjustIdId.value)

    exportButtonFunction()

    // this.$router.push('/countStock/CountAdjustStock/countStock')
  } catch (error) {
...
const redirectToNextPage = () => {
  ...
  isDialogVisibleCancelSuccess.value = false

  // console.log('redirectToNextPage Functioned Success')
  getCountingStock()
  to.replace( '/pages/user-profile')
}
...
const testRepalce = () => {
  ...
  router.replace('/countStock/CountAdjustStock/countStock')
}
...
const convertCategoryName = dataSpaceConvert => {
  return dataSpaceConvert.replace(/\s/g, '<br />')
}
...
console.log('%ccheckConfigShowRFID:', 'color: white; font-size: 16px; background-color: red', checkConfigUser(configShowRfid.value))



// watch(shouldShowRfid, (newValue, oldValue) => {
//   // Handle changes to shouldShowRfid
//   console.log('shouldShowRfid changed:', newValue)
// })

// console.log('checkConfigUser: ', configShowRfid.value)
...
<template>
            ...
            <template #opposite>
              <RouterLink
                :to="{ 
                  ...
                  query: { stockAdjustId: StockAdjustIdId ,zoneId:searchByZoneId,warehouseStock: whereHouse,status:statusAdjustStock }, 
                }"
              >
                {{ $t('Counting') }}
              </RouterLink>
  ...
  <!-- Dialog for Alert When Btn Start/Cancel Success or Not Success -->
  <section>
    <!-- Dialog Btn Start Success -->
    <VDialog
    ...
    >
      ...
      <VCard>
        ...
        <div class="py-4 text-center">
          ...
          <VCardText>
            <span class="text-h5">{{ $t('Count/Adjustment Successfully Started') }}</span>
          </VCardText>
        </div>
      </VCard>
    </VDialog>
    <!-- Dialog Btn Start Not Success -->
    <VDialog
      v-model="isDialogVisibleStartNotSuccess"
      persistent
      class="v-dialog-sm"
    >
      <!-- Dialog Content -->
      <VCard>
        <DialogCloseBtn
          variant="text"
          size="small"
          @click="isDialogVisibleStartNotSuccess = false"
        />
        ...
        <div class="py-4 text-center">
          ...
          <VCardText>
            <div>
              <span class="text-h5">{{ $t('Counting/Adjustment Did Not Start Successfully') }}</span>
            </div>
            <div>
              <span>**{{ $t('alertBtnStartNotSuccess') }}</span>
            </div>
          </VCardText>
        </div>
      </VCard>
  ...
  <section>
    <VRow>
      <VCol
        cols="6"
        class="mt-2 d-flex justify-center justify-lg-start justify-sm-start"
        lg="2"
      >
        <!-- Btn Start  -->
        <VBtn
      ...
      >
        <!-- Btn Clear  -->
        <VBtn
          :disabled="isDialogVisibleCountAgain"
          class="mx-1 custom-btn-start-dialog"
          variant="outlined"
