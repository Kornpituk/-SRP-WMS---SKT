...
const saveBarcode = async index => {
  try {
    const product = products.value.items[index]
    ...
    const requestData = {
      ...
      qtyAdj: product.qtyAfter,
    }

    const response = await axiosIns.post(
      `${urlApi.value}/api/v1/CountingStock/Barcode/Save/Item/Web`,
      requestData,
      {
        headers: {
          'accept': '*/*',
...
const saveBarcodeWebAdjustID = async () => {
  try {
    const response = await axiosIns.post(
...
const shouldDisableSaveButton = () => {
  if (isConfirmRfid.value === true && isConfirmBarcode.value === false) {
  ...
  <section>
    <VRow>
      ...
      <VCol
        cols="12"
        lg="2"
        class="mt-2 d-flex justify-center justify-lg-start justify-sm-start custom-btn"
      >
        ...
        >
          ...
          <template #activator="{ props }">
            <VBtn
              ...
              color="green"
              v-bind="props"
              :disabled="isConfirmRfid === true && isConfirmBarcode === false ? false : true"
              @click="saveBarcodeWebAdjustID"
              size="large"
              style="width: 100%;"
            >
              <span style="font-size: 16px;">{{ $t('Save') }}</span>
            </VBtn>
          </template>
          ...
          <VCard>
            ...
            <div class="py-4 text-center">
              ...
              <VCardText>
                <span class="text-h5">{{ $t('Barcode/counting successfully recorded') }}</span>
              </VCardText>
            </div>
          </VCard>
        </VDialog>
        <!-- --------------------- Btn Confirm When Error ----------------------- -->
        <VDialog
          v-model="isDialogVisibleError"
          persistent
          class="v-dialog-sm"
        >
          <!-- Dialog Content -->
          <VCard>
            <DialogCloseBtn
              variant="text"
              size="small"
              @click="isDialogVisibleError = false"
            />
            ...
            <div class="py-4 text-center">
              ...
              <VCardText>
                <span class="text-h5">{{ $t('Barcode/counting not success recorded') }}</span>
              </VCardText>
            </div>
          </VCard>
        </VDialog>
      ...
      <VTable class="text-no-wrap table-header-bg rounded-0">
        ...
        <tbody>
          ...
          >
            ...
            <td
              class="text-end"
              :class="{'edited-cell': product.qtyDif == 0, 'positive-dif': product.qtyDif > 0, 'negative-dif': product.qtyDif < 0 }"
            >
              <VTextField
                v-model="product.qtyAfter"
                density="compact"
                @keyup.enter="saveBarcode(index)"
              />
            </td>
            <!-- 👉 barcodeQtyDif -->
            <td
              class="text-end"
              :class="{ 'green-dif': product.qtyDif === 0, 'red-dif': product.qtyDif < 0, 'blue-dif': product.qtyDif > 0 }"
            >
