...
import { useCurrencyActions } from './composables/useCurrencyActions'
...
const { rows, loading, error, loadRows } = useCurrencyData()
...
const {
  ...
  closeDeleteDialog,
  removeItem,
} = useCurrencyActions(applySearch, setError)
...
<template>
  <VContainer fluid>
    ...
    >
      <VCard>
        <VCardTitle>{{ form.id ? 'Edit' : 'Create' }} Currency</VCardTitle>
        <VCardText>
          <VRow>
            <VCol
              cols="12"
    ...
    >
      <VCard>
        <VCardTitle>Delete Currency</VCardTitle>
