...
import { useShippingModeActions } from './composables/useShippingModeActions'
import { useShippingModeData } from './composables/useShippingModeData'

const search = reactive({
  keyword: '',
})

const { rows, loading, error, loadRows } = useShippingModeData()
...
const {
  ...
  closeDeleteDialog,
  removeItem,
} = useShippingModeActions(applySearch, setError)
...
<template>
  <VContainer fluid>
    ...
    >
      <VCard>
        <VCardTitle>{{ form.id ? 'Edit' : 'Create' }} Shipping Mode</VCardTitle>
        <VCardText>
          <VRow>
            <VCol
              cols="12"
    ...
    >
      <VCard>
        <VCardTitle>Delete Shipping Mode</VCardTitle>
