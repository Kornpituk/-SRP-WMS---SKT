...
import { usePortData } from './composables/usePortData'

const search = reactive({
  keyword: '',
})

const { rows, loading, error, loadRows } = usePortData()
...
const {
  ...
  closeDeleteDialog,
  removeItem,
} = usePortActions(applySearch, setError)
...
<template>
  <VContainer fluid>
    ...
    >
      <VCard>
        <VCardTitle>{{ form.id ? 'Edit' : 'Create' }} Port</VCardTitle>
        <VCardText>
          <VRow>
            <VCol
              cols="12"
    ...
    >
      <VCard>
        <VCardTitle>Delete Port</VCardTitle>
