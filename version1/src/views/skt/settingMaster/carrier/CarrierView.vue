...
import { useCarrierActions } from './composables/useCarrierActions'
import { useCarrierData } from './composables/useCarrierData'

const search = reactive({
  keyword: '',
})

const { rows, loading, error, loadRows } = useCarrierData()
...
const {
  ...
  closeDeleteDialog,
  removeItem,
} = useCarrierActions(applySearch, setError)
...
<template>
  <VContainer fluid>
    ...
    >
      <VCard>
        <VCardTitle>{{ form.id ? 'Edit' : 'Create' }} Carrier</VCardTitle>
        <VCardText>
          <VRow>
            <VCol
              cols="12"
    ...
    >
      <VCard>
        <VCardTitle>Delete Carrier</VCardTitle>
