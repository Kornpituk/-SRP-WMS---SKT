...
import { useSalePersonActions } from './composables/useSalePersonActions'
import { useSalePersonData } from './composables/useSalePersonData'

const search = reactive({
  keyword: '',
})

const { rows, loading, error, loadRows } = useSalePersonData()
...
const {
  ...
  closeDeleteDialog,
  removeItem,
} = useSalePersonActions(applySearch, setError)
...
<template>
  <VContainer fluid>
    ...
    >
      <VCard>
        <VCardTitle>{{ form.id ? 'Edit' : 'Create' }} Sale Person</VCardTitle>
        <VCardText>
          <VRow>
            <VCol
              cols="12"
    ...
    >
      <VCard>
        <VCardTitle>Delete Sale Person</VCardTitle>
