...
import { useForwarderActions } from './composables/useForwarderActions'
import { useForwarderData } from './composables/useForwarderData'

const search = reactive({
  keyword: '',
})

const { rows, loading, error, loadRows } = useForwarderData()
...
const {
  ...
  closeDeleteDialog,
  removeItem,
} = useForwarderActions(applySearch, setError)
...
<template>
  <VContainer fluid>
    ...
    >
      <VCard>
        <VCardTitle>{{ form.id ? 'Edit' : 'Create' }} Forwarder</VCardTitle>
        <VCardText>
          <VRow>
            <VCol
              cols="12"
    ...
    >
      <VCard>
        <VCardTitle>Delete Forwarder</VCardTitle>
