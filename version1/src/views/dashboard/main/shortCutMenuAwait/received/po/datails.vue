...
const GetReceivedPoDetails = async () => {
  ...
  try {
    const response = await axiosIns.get(`${urlApi.value}/api/v1/Dashboard/Performance/ReceivePo/Detail/Await?page=` + currentPage.value + `&perPage=` + rowPerPage.value, {
      headers: {
        'accept': '*/*',
    ...
    loadingFetchDetails.value = false // ✅ ย้ายมาไว้ใน finally เพื่อให้ทำงานเสมอ
  }
}
