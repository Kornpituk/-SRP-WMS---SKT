...
const errorMessage = ref(null)          // ✅ เพิ่มบรรทัดนี้
...
const GetReceivedPoDetails = async () => {
  ...
  try {
    const response = await axiosIns.get(`${urlApi.value}/api/v1/Dashboard/Performance/PickingDelivery/Detail/Await?page=`+currentPage.value+`&perPage=`+rowPerPage.value, {
      headers: {
        'accept': '*/*',
  ...
  <section>
    ...
    >
      <VCardTitle class="text-center bg-amber">
        <span class="text-white">Delivery : Await Picking </span>
      </VCardTitle>
