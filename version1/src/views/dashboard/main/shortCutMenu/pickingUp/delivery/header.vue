...
const sortByColPickingDateData = ref(null)
const sortByColRequestDateData = ref(null)
const sortByColDeliveryDateData = ref(null)
...
// เปลี่ยนชื่อและประเภทของตัวแปรใหม่
...
const GetReceivedPoHeader = () => {
  ...
  // console.log('searchByCategoryName: ',searchByCategoryName)
  axiosIns.get(`${urlApi.value}/api/v1/Dashboard/Performance/PickingDelivery/Header?page=`+currentPage.value+`&perPage=`+rowPerPage.value, {
    headers: {
      'accept': '*/*',
      'x-location': `${searchByWareHouseId.value}`,
      Authorization: `Bearer ${accessTokenAtStore}`,
                      ...
                      <VRow>
                        ...
                        >
                          <VBtn
                            type="submit"
                            style="width: 100%;"
                            @click="menuRequestWarehouse = false"
                          >
                            {{ $t('Cancel') }}
                          </VBtn>
                        </VCol>
