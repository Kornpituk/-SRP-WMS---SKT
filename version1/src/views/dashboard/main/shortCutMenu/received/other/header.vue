...
const refDocData = ref(null)
const reasonData = ref(null)
const remarkData = ref(null)

const receiveDateData = ref(null)

const receiveBy = ref(null)
...
const menuSubplierName = ref( false)
...
const GetReceivedPoHeader = () => {
  ...
  // console.log('searchByCategoryName: ',searchByCategoryName)
  axiosIns.get(`${urlApi.value}/api/v1/Dashboard/Performance/ReceiveOther/Header?page=`+currentPage.value+`&perPage=`+rowPerPage.value, {
    headers: {
      'accept': '*/*',
      'x-location': `${searchByWareHouseId.value}`,
      Authorization: `Bearer ${accessTokenAtStore}`,
    ...
    // dateSp: dateSpData.value,
      ...
      receiveBy: receiveBy.value,

      sortByColReceiveDate: sortByColRerceiveDate.value,
          ...
          <tr>
            ...
            >
              ...
              <VIcon
                ...
                @click="sortByColRerceiveDate = sortByColRerceiveDate === 'asc' ? 'desc' : 'asc'"
              />
            </th>
            <th
              scope="row"
