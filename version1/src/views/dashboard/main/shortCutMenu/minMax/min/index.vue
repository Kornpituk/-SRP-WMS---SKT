...
const props = defineProps({
  ...
  test: {
    type: String, // หรือประเภทของข้อมูลที่ต้องการรับ
...
const toggleSortType = sortBy => {
  const sortRefs = { sortByColMin, sortByColMax, sortByColTotalQty, sortByColLastupdate }
...
const searchColumns = {
  searchColCategory,
  searchColTypeName,
  searchColSubTypeName,
  searchColBarcode,
  searchColProductId,
  searchColProductName,
  searchColUoM,
}
...
const clearValues = () => {
  ...
  for (const key in searchColumns) {
    searchColumns[key].value = null
  }

  // Reset sort columns
  for (const key in sortColumns) {
    sortColumns[key].value = null
  }
...
const GetStockUpdateForPagination = () => {
  ...
  // console.log('searchByCategoryName: ',searchByCategoryName)
  axiosIns.get(`${urlApi.value}/api/v1/Dashboard/Summary/StockMaxMin/Min`, {
    headers: {
      'accept': '*/*',
      'x-location': `${searchByWareHouseId.value}`,
      Authorization: `Bearer ${accessTokenAtStore}`,
    ...
    .then(response => {  
      ...
      console.log('[products.value]!!: ', response.data.items)
...
const fetchItemsCatSearchBy = () => {
  return axiosIns.get(`${urlApi.value}/api/v1/Product/categories`, {
    headers: {
      'accept': '*/*',
    ...
    itemsSearchByCategoryId.value = response.data
  }).catch(error => {
    console.error('Error:', error)
      ...
      <VExpansionPanel>
        ...
        >
          <template #default="{ expanded }">
            ...
            >
              <div v-if="expanded">
                <span class="text-h6">{{ $t('Filter') }}</span>
              </div>
              <span
                v-if="!expanded"
                class="text-h6"
            ...
            <VRow>
              ...
              >
                <VAutocomplete
                  ...
                  :placeholder="$t('Product Sub Group')"
          ...
          <tr>
            ...
            >
              ...
              <VIcon
                ...
                @click="toggleSortType('sortByColMin')"
              />
            </th>
            <th
              scope="row"
        ...
        <tbody>
          ...
          >
            ...
            <td
              ...
              class="text-end  px-6"
            >
              {{ formatDecimal(product.minQty).toLocaleString('en-US') }}
            </td>
            <!-- 👉 Number(Tag) -->
            <td
              v-if="checkConfigUser(nameUser)"
