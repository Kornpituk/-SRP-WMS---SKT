...
localStorage.setItem('productIdForGenCode', '')
...
const saveProductData = async () => {
  ...
  try {
    const response = await axiosIns.post(`${urlApi.value}/api/v1/Product/Save?`, formData, {
      headers: {
        'accept': '*/*',
        'x-location': `${whereHouse}`,
        Authorization: `Bearer ${accessTokenAtStore}`,
    ...
    >
      ...
      <VCard>
        ...
        <div class="py-4 text-center">
          ...
          <VCardText>
            <span class="text-h5">{{ $t('Complete Set Product Details!') }}</span>
          </VCardText>
        </div>
      </VCard>
    </VDialog>
      ...
      <VRow class="d-flex justify-end">
        ...
        >
          <VBtn
            ...
            color="red"
          >
            <span style="font-size: 16px;">{{ $t('Cancel') }}</span>
          </VBtn>
        </VCol>
        <VCol
          cols="6"
